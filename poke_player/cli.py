...
Usage:
    poke-player serve --rom path/to/rom.gba [--port 8765] [--data-dir ~/.poke-player]
    poke-player info  --rom path/to/rom.gba
    poke-player --version
"""
...
def cmd_serve(args):
    ...
    from poke_player.server import GameConfig, configure, app  # noqa: F811
...
def main():
    ...
    parser.add_argument(
        "--version", action="version", version=f"poke-player {__version__}"
    )
    ...
    serve_p.add_argument(
        "--data-dir", default="~/.poke-player",
        help="Data directory for saves, etc. (default: ~/.poke-player)",
    )
    serve_p.add_argument(
